...
*/
int readtag (CS203DC_Handle *handle, char **my_tag_info) {
  ...
  // printf("\nrecv: %d with errno: %d\n", nrecv, myerrno);
   ...
   {  
	...
	//	printf ("\nPrinting inside func\n%s\n", toSerialBuff);
		memmove(*my_tag_info, toSerialBuff, 1600 * sizeof(char));
...
void *run_rfid (void *fd, char **my_tag_info)
{
...
//	while ((time (NULL) -  start_time) < 20 || CS203fd->iConf_Forever == 1) 
	...
	{
		int result = readtag (CS203fd, my_tag_info);
 ...
 **
 *****************************************************************************/
CS203DC_Handle *init_rfid (int ac, char *av[])
{
...
//	CS203DC_Handle *CS203fd = fd;
	...
	if (ac < 2)
	{
		...
		printf("-toggle : 1=Gen 2 A/B flag will be toggled after all arounds have been uun on current target, 0=no toggle\n");
