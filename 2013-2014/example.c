...
*/
	...
	// retrieve attached radio list (intel command), to check if the board is turnt on
...
//#endif
		...
		//printf("lol\n");
			...
			if (memcmp (pRecvBuffer, "\x98\x98\x98\x98\x98\x98\x98\x98", 8) == 0)
			{
				...
				//printf("pRecvBuffer (pre): %d\n", pRecvBuffer);
				pRecvBuffer += 8;
				nread -= 8;
				printf("pRecvBuffer: %d\n", pRecvBuffer);
				//goto ian;
				continue;
			}

printf("SOMETHING HAPPENED: %x\n", PacketType);
printf("PacketLength vs nread: %d vs %d\n", PacketLength, nread);
...
printf ("%s(): line: %d: <Check CS203 IP : %x>\n", __FUNCTION__, __LINE__, handle->ipv4addr);
  	fcntl (sockfd, F_SETFL, O_NONBLOCK | FASYNC);
...
unsigned int GetLocalIP ()
{
   ...
   {
		printf ("getifaddrs failed in GetLocalIP()\n");
   		return ret;
   }
...
printf(" ifa->ifa_name: %s  ||", ifa->ifa_name);
		if (strcmp (ifa->ifa_name, "en0") == 0 && family == AF_INET)
		{
printf("eth0  ||");
...
printf("**SETTRUSTEDSERVER\n");
 ...
 // Check Mode
 printf("**GETMODE (apparently wants to be low level)\n");
...
printf("INVENTORY STOPPED || tagid: %d\n", tzTagID);
...
CS203DC_Handle *global_fd;

void mySignalHandler(int signo) {
printf("SHUT IT DOWN!\n");

//CS203DC_StopInventory(global_fd);
CS203DC_Close(global_fd);
CS203DC_Reboot(global_fd->ipv4addr, DEFAULT_UDPPORT);
...
signal(SIGINT, mySignalHandler);
	...
	global_fd = CS203fd_first = handler_alloc ();
